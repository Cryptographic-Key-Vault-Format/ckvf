...
- **MUST NOT** silently redefine CKVF behavior to match Swift conventions or a vendor product.
...
Intended public API (see `Sources/CKVF/CKVF.swift`): parse, validate, create, encrypt, decrypt, merge, and identifiers (`identityId`, `mskId`, `absoluteKeyId`, Short Key ID).

## Status

Calls currently `fatalError`. Do not treat a future green CI badge as conformance until real tests pin a test-vector `VERSION`.
